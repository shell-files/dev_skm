...
| 현재 구현 여부 | 부분 구현 |
| 부족 구현 | `matrixItems`, `topIssues`, `selectedYn`, `quadrant`, `nextStep`, `selectionReasons`, `requiredMetricCount`, `onboardingMissingCount`, `highPriorityCount` |
...
class ReportTraceResponseDto(BaseModel):
    runId: int
    paragraphId: int
    traceType: Literal["DIRECT", "GROUP_ROLLUP"]
    sourceModeLabel: str
    metrics: list[dict]
