...
from jobs.core.utils import createRow
import gc
...
def crawl(driver):
    ...
    while True:
        ...
        articles = driver.find_elements(
            By.CSS_SELECTOR, "#section-list ul.type1 > li")
        ...
        for article in articles:
            try:
                dateText = article.find_element(
                    By.CSS_SELECTOR, ".info.dated").text.strip()

                # 날짜 파싱
                try:
                    if len(dateText.split('.')[0]) <= 2:  # "05.13 14:29"
                        articleDate = datetime.strptime(
                            f"{currentYear}.{dateText}", "%Y.%m.%d %H:%M")
