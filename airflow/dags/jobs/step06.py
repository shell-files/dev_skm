...
from jobs.core.subissuemaster import subissueMaster
from jobs.core.paths import embeddedJsonl, similarityJsonl, subissueVectorJsonl
...
SBERT_INPUT_FILE = os.path.join(os.path.dirname(
    embeddedJsonl), "embeddedEsgChunks_sbert.jsonl")

# 출력 파일명들은 깔끔하게 카멜 케이스 규칙을 유지하시면 됩니다.
SBERT_OUTPUT_FILE = os.path.join(os.path.dirname(
    similarityJsonl), "similaritySbert.jsonl")
SBERT_SUBISSUE_VECTOR_FILE = os.path.join(os.path.dirname(
    subissueVectorJsonl), "subissueVectorSbert.jsonl")
...
def processSimilarityPipeline(modelName, inputFile, outputFile, subissueVectorFile, cacheFolder=None):
    ...
    subissueVectors = getSubissueVectors(
        model, subissueVectorFile, forceRebuild=True)
    ...
    with open(outputFile, "w", encoding="utf-8") as outF:
        for record in records:
            ...
            topMatches = findTopMatches(
                chunkEmbedding, subissueVectors, topK=3)
