...
from jobs.core.paths import similaritySbert


def step07(**context):
    pg_hook = PostgresHook(postgres_conn_id='Vector')
    ...
    try:
        ...
        cursor.execute("TRUNCATE TABLE esg_chunks")
        cursor.execute("alter sequence esg_chunks_id_seq restart with 1")
        conn.commit()
        print("✅ 데이터베이스 스키마 준비 완료!")

        # (유사도 매칭 점수와 최고 매칭 subIssueId가 결합된 최종 결과 파일)
        similarityFile = similaritySbert
        ...
        with open(similarityFile, "r", encoding="utf-8") as f:
            for line in f:
                ...
                text_to_search = (record.get("chunk", "") +
