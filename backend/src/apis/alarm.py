...
async def alarmWebSocket(
    ...
    companyId  : int = Query(..., description="현재 선택된 회사 ID"),
):
    # utils/websc.py — 인증 처리
    userId = await authenticateWS(websocket, token)
    if not userId:
        return

    # utils/websc.py — 연결 등록
    await manager.connect(websocket, userId, companyId)
    try:
        while True:
        ...
        manager.disconnect(userId, companyId)
...
def getAlarms(
    ...
    companyId      : int = Header(..., alias="CompanyId"),
):
    return getAlarmListProcess(alarmListModel, companyId)
...
def patchAlarm(
    ...
    return readAlarmProcess(alarmReadModel, companyId)
...
def alarmDel(
    ...
    return deleteAlarmProcess(alarmId, uuid, companyId)
