...
class RollupReadApiServiceTest(unittest.TestCase):
    def test_list_requests_preserves_history_options(self):
        ...
        with patch.object(service, "loadRepository", return_value=fakeRepo), patch.object(service, "getSource", return_value=7):
        ...
        self.assertEqual(response.data.items[0].metricCount, 2)
    ...
    def test_request_detail_uses_source_scope_and_snapshot_counts(self):
        ...
        self.assertEqual([item.metricId for item in data.items], ["M1", "M2"])
