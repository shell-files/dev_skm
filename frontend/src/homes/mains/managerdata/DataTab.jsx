/**
 * DataTab.jsx
 * 데이터 승인 탭 컴포넌트 — ManagerData.jsx 내에서 사용.
 * E/S/G/General 도메인 탭별 메트릭 데이터 테이블 + 승인/검토/제출 액션 제공.
 *
 * props:
 *   items          — 표시할 메트릭 아이템 목록 (mapApprovalItemToInput 변환 완료 상태)
 *   onAction       — 승인 액션 콜백 (actionType, itemId, comment)
 *   readOnlyYn     — true이면 완료 프로젝트로 모든 액션 비활성화
 *   isLoading      — 로딩 중 여부 (스켈레톤 표시)
 *
 * 의존:
 *   dataTabUtils.js — DOMAIN_ORDER, 상태 정규화 헬퍼, 레이블 함수
 */
...
import {
  PAGE_SIZE, DOMAIN_ORDER, DOMAIN_LABEL,
  safeArray, normalizeIssueDomain, normalizeSubIssueCode, normalizeSubIssueName,
  normalizeApprovalStatus, normalizeSubmitStatus, normalizeReviewStatus,
  isActionSupported, actionDisabledTitle, submitLabel, reviewLabel, approvalLabel,
} from "./dataTabUtils";
